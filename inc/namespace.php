...
function bootstrap() : void {
	add_action( 'registered_post_type', __NAMESPACE__ . '\\post_type_registration_action', 10, 2 );
 ...
 */
function post_type_registration_action( string $post_type, WP_Post_Type $post_type_object ) : void {
 ...
 */
function pre_insert_in_rest( stdClass $prepared_post, WP_REST_Request $pre_request ) : stdClass {
 ...
 */
function is_rest( $toggle = null ) : bool {
	static $in_rest = 0;
 ...
 */
function populated_transition_post_status( string $new_status, string $old_status, WP_Post $post ) : void {
 ...
 */
function populated_post_updated( int $post_id, WP_Post $post_after, WP_Post $post_before ) : void {
 ...
 */
function populated_add_attachment( int $post_id ) : void {
	if ( ! is_rest() ) {
 ...
 */
function populated_insert_post( int $post_id, WP_Post $post, bool $update ) {
	if ( ! is_rest() || ! get_post_type_object( get_post_type( $post_id ) )->show_in_rest ) {
